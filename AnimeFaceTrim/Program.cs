	...
	class Program
	{
		static void Main(string[] args)
		{
			try
			{
				...
				string outfile = Path.Combine(Path.GetDirectoryName(filepath), Path.GetFileNameWithoutExtension(filepath) + "_trim" + Path.GetExtension(filepath));
				...
				detectFaceImage.SaveImage(outfile);

				Console.WriteLine("Detect Face Success. " + outfile);
			}
			catch(Exception)
			{
		...
		static Mat DetectFaceInImage(Mat image, string cascade_file)
		{
			...
			//　元写真から検知部分を切り抜き
			return image[ny, rowEnd, nx, colEnd];
		}
	}
}
